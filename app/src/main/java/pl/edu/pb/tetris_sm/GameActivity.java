...
public class GameActivity extends Activity implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        soundPool = new SoundPool.Builder()
                ...
                .build();
        line = soundPool.load(this, R.raw.coin, 1);
        ...
        language = prefs2.getString("My_Lang", "");
        ...
        textView.setText(getResStringLanguage(R.string.game_over_textview_string, language));
        textView.setVisibility(View.INVISIBLE);
        TextView textView2 = (TextView) findViewById(R.id.game_over_textview2);
        textView2.setText(getResStringLanguage(R.string.game_over_textview_string2, language));
    ...
    void PaintMatrix() {
        ...
        if (!gameInProgress) {
            ...
            canvas.drawText(getResStringLanguage(R.string.paused, language), (float) (BOARD_WIDTH / 2.0), (float) (BOARD_HEIGHT / 2.0), paint);
        }
        ...
        String scoreText = getResStringLanguage(R.string.game_score_string, language);
        textView.setText(scoreText + score);

        // Change difficulty level when player gets more points
        TextView textView2 = (TextView) findViewById(R.id.game_difficulty);
        if (score == 0) {
            String diffText = getResStringLanguage(R.string.game_difficulty, language);
            textView2.setText(diffText);
        }
        else if (score >= 1 && score < 5) {
            playSfx("lvlup");
            SPEED_NORMAL = 500;
            SPEED_FAST = 50;
            String diffText = getResStringLanguage(R.string.game_difficulty2, language);
            textView2.setText(diffText);
        }
        else if (score >= 5) {
            playSfx("lvlup");
            SPEED_NORMAL = 250;
            SPEED_FAST = 25;
            String diffText = getResStringLanguage(R.string.game_difficulty3, language);
            textView2.setText(diffText);
        }
        if (gamePaused) {
            pauseMusic();
    ...
    // close this activity and run game over activity
    @Override
    public boolean onSingleTapUp(MotionEvent e) {
        if (!gameInProgress) {
            Intent intent = new Intent(GameActivity.this, GameOverActivity.class);
            GameOverActivity.finalScore = score;
            finish();
        ...
        if (SettingsActivity.musicEnabled) {
            if (player == null) {
                if (Math.random() < 0.5)
                    ...
                    player = MediaPlayer.create(this, R.raw.korobeiniki);
    ...
    public void playSfx(String which) {
        if (SettingsActivity.musicEnabled) {
            switch (which) {
                case "line":
                    soundPool.play(line, 1, 1, 0, 0, 1);
                    break;
                case "gameover":
